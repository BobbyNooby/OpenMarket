...
export const conversationRoutes = new Elysia()
  ...
  .post(
    ...
    async ({ body, session, set }) => {
      ...
      // Check if a conversation already exists for this user pair (one chat per pair, regardless of listing context)
      ...
      if (myConvIds.length > 0) {
        ...
        if (sharedConvIds.length > 0) {
          const [existing] = await db
            ...
            .where(inArray(conversationsTable.id, sharedConvIds))
            .limit(1);
